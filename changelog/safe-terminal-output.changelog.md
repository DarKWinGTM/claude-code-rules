...
| | | Summary: Re-materialized root `safe-terminal-output.md` as a substantive active runtime rule with `Full history`, preserving current design target behavior while removing design-only runtime dependency | |
...
### Changes
- Updated `safe-terminal-output.md` from v1.3 to v1.4.
- Updated `design/safe-terminal-output.design.md` from v1.3 to v1.4.
...
### Summary
Safe terminal output now has an installed active runtime body again, so the root rule can carry runtime behavior directly instead of relying on a design pointer as a substitute.
