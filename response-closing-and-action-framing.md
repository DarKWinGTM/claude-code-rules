...
Future-work ideas must stay clearly advisory unless the user selects them. A useful proposal states concrete goal, expected improvement/change, expected output or user-visible result, and success condition when it clarifies what done means.
Required guidance:
...
Other options: go straight to implementation now; pause after the current source-only update.
