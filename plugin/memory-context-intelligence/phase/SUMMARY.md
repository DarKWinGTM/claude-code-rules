...
- current proof for the analysis surface now comes from the RULES-owned packaged `intake → signals → present` chain, an added packaged recall-only no-promotion proof, green focused tests, a green `98`-test full package suite, one approved local slash proof without config showing the guided helper, and one approved local slash proof with auto-discovered config showing no-args policy loading
