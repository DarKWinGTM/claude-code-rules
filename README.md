...
| [`execution-continuity-and-mode-selection.md`](execution-continuity-and-mode-selection.md) | Execution continuity and mode selection | Separates discussion mode from execution mode, keeps startup artifact governance as an explicit precondition before execution continuity takes over, keeps work moving when execution mode is active, and discovers the next unfinished slice from execution surfaces when that path is already visible |
...
The Quick Start block above is still the canonical runtime-only install block. The methods below use the same active 41-rule set and describe when to use each path without repeating the long file list.
