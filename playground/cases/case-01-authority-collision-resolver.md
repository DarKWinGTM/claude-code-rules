...
- Keep durable objective/Team expansion advisory until selected.
- Invoke only the smallest justified bounded helper topology inside selected work, under `worker-routing-and-context.md`, without widening objective or authority.
...
The latest valid user instruction controls the objective unless a hard boundary blocks it. New durable expansion remains advisory, while bounded helper support inside that selected objective follows the canonical routing owner.
...
- Keep any new objective or durable Team proposal unselected until the user chooses it.
- When the selected objective already implies one bounded helper lane, invoke it internally under the routing owner and keep Leader/Main responsible for integration and proof.
...
Without this rule family, the assistant can continue from stale framing, treat a proposed durable expansion as selected, or make the user choose bounded internal routing mechanics.

With RULES active, stale framing is retired, durable expansion stays user-owned, and the smallest justified helper topology can support the selected objective without becoming new authority.
