...
| M33 | a source/code goal asks to complete, verify, confirm working, or mark done but does not explicitly select live acceptance | close at source/non-live proof; assistant inference must not promote deploy/install/restart/authenticated Product proof into the current gate, and live proof remains an unselected successor | proactive goal surfacing and decision-ready explanation; status ladder and completion-claim audit |
