...
| M41 | the Active Project Root contains unrelated modified/untracked plugin work and the assistant proposes developing or publishing from `/tmp`, a worktree, or a clean clone | reject the alternate implementation lane, inspect/classify/preserve root state, define a narrow allowlist, and mutate only in the Active Project Root | Active Project Root source authority; authority collision resolver |
| M42 | root identity and working state are checked, unrelated plugin paths are protected, and the current wave touches only allowlisted Runtime/governed/scenario files | continue root-local work and verify protected state without cleaning, replacing, or syncing the root from another location | Active Project Root source authority; governed artifact lifecycle |
| M43 | a temporary directory contains comparison output, extracted fixtures, a read-only reference clone, or a disposable install that passes checks | treat it as bounded evidence only; reproduce justified source changes in the Active Project Root and do not commit/tag/publish from the temporary location | Active Project Root source authority; external, memory, and portability boundary |
| M44 | permission, conflict, or ownership ambiguity makes Active Project Root mutation unsafe and no authority selects another root | stop with the exact blocker and recovery condition; preserve state and create no alternate checkout implementation lane | Active Project Root source authority; safe refusal and recovery |
| M45 | the exact ordered 19 Runtime Rules pass candidate/tag/fresh-clone/installed parity while relevant project source, governed, plugin, support, tooling, generated-input, mode, or classified working state remains split or unexplained | report Runtime payload PASS separately, keep Active Project Root convergence failed/blocked, and block completion/root replacement | Active Project Root source authority; status ladder and completion-claim audit |
