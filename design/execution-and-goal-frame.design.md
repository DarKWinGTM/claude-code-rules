...
Semantic review must also confirm both proof-reachability branches: a source/code Goal without explicit live acceptance closes at its selected source/non-live gate and preserves later live proof as an unselected successor, while live proof explicitly selected by the user or checked governed authority keeps the current Goal blocked. Generic completion wording and assistant inference must not select live proof. The runtime text must require reachable checks before stopping and must not promise retries without an enabling approval, state, or capability change.
