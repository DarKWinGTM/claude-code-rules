...
- Preserve explicit gate classification when the implementation subject uses operational nouns or Product-facing capability names; capability naming alone cannot select live execution or mutation proof.
- Repair assistant-authored infeasible done points in place when they were never explicitly selected: retain reached proof, move the impossible point to excluded or successor scope, reconcile tasks, and avoid new-Goal or unchanged retry loops.
