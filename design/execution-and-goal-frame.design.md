...
P123 refinement: this owner should now preserve that selected or promoted governed `/goal` work may use internal native subagent assistance for analysis, verification, testing, and full plan-file preparation without creating a new user-facing command, while still keeping `/goal` as the objective owner, keeping the referenced plan file as route-only support, and keeping leader-owned synthesis/proof wording as the completion authority.

P124 refinement: this owner should now require advisory governed `/goal` creation to run an internal pre-goal planning pass before final goal emission, let native subagents help with analysis, route drafting, verification ordering, and plan-file preparation, require the emitted goal to reference that plan file, and treat any direct/simple-goal wording as applying only to non-governed lightweight next-step guidance rather than created governed goals.

P125 refinement: this owner should now preserve one integrated goal-with-plan-reference visible surface so governed `/goal` work keeps the emitted goal compact, keeps the detailed route in the referenced plan file, and reserves `/plan` for explicit standalone planning, later route revision, or overflow around an already plan-backed selected goal instead of the ordinary paired next surface.

P130 refinement: this owner should now require plan-first authoring for any actual governed `/goal` creation: prepare a full detailed plan file before final goal emission, keep `/goal` as the objective owner, require the emitted `/goal` to reference that plan file as route-only support, keep the emitted `/goal` compact even when the referenced plan is detailed, and preserve `/plan` only for explicit standalone planning, later route revision, or overflow around an already plan-backed selected goal.

P121 refinement: this owner should now preserve an explicit goal-to-plan bridge only for explicit standalone planning, later route revision, or overflow around an already plan-backed selected goal, while keeping `/goal` as the objective owner and keeping route completion weaker than goal-gate closeout.

P122 refinement: this owner should now preserve explicit `/plan` next-surface recommendation only when standalone planning is explicitly needed after goal selection or when later route revision/overflow exceeds the selected goal plus its referenced plan file.
