...
- one short prompt ↔ AI response example dialogue, clearly labeled when it is virtual
...
The baseline playground family should include:
...
Required baseline scenario count for this wave:
- 10 scenario-family case files

---

## Scenario Coverage Contract

The playground baseline must:
- define 10 scenario families that are grounded in current RULES behavior
- map all 18 active runtime rule files to at least one scenario family
- keep the scenario-family naming semantic rather than version-shaped
- allow one rule to appear in several scenario families when that better reflects real behavior

Recommended scenario families for the baseline:
...
2. record the checked evidence in the current monthly observed log
3. update the relevant case file's observed section or observed-count note
4. open a new scenario family only when the current ten no longer cover the behavior honestly
...
Contract:
- README keeps a pointer-level section or equivalent navigation hook
- scenario bodies, matrices, and observed logs live under `playground/`
- README must not absorb the playground into a broad before/after or marketing dump
...
- `playground/matrix.md` uses several explicit virtual-case axes
...
- richer observed logs or comparative analysis views
