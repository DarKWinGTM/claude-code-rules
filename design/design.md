...
- advisory `/goal` creation for governed non-trivial or route-heavy work may conditionally run an internal pre-goal planning pass before final goal emission when route synthesis materially improves the command
...
- main-controller synthesis, visible proof wording, and goal-gate closeout remain leader-owned even when helper lanes or pre-goal route drafts contribute evidence
