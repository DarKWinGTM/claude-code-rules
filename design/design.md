...
- native worker-agent routing and context control keeps broad, noisy, high-context, multi-surface, high-output, or naturally parallel work from defaulting to leader-session raw absorption; worker routing decides the smallest effective worker structure, and custom-agent selection chooses the best available specialist only after that routing decision
...
| 7 | custom-agent-selection-priority.md | custom-agent-selection-priority.design.md v1.2 | First-class owner for preferring visible user custom agents as the primary specialist pool after worker routing has already determined that delegation or specialist handling is appropriate; it selects the best-fit specialist/custom agent without becoming the owner of worker-scale routing or leader-context control |
...
| 20 | native-worker-agent-routing-and-context-control.md | native-worker-agent-routing-and-context-control.design.md v1.0 | First-class owner for workload-shaped worker routing and leader-context control: broad, noisy, context-heavy, high-output, multi-surface, or naturally parallel work should pass through the smallest effective direct/subagent/multi-subagent/Agent Team decision before leader-session raw absorption, while worker handoffs stay analyzed and proportionate and leader verification remains mandatory |
...
| 39 | execution-continuity-and-mode-selection.md | execution-continuity-and-mode-selection.design.md v1.8 | First-class owner for discussion-vs-execution mode selection, startup-gate-first execution boundaries, continuous-execution defaults after startup posture is resolved enough, active next-work discovery from execution surfaces, capture-before-continue protection when implementation-critical external knowledge has not yet been normalized into governed artifacts, worker-routing-before-broad-continuation protection so broad next slices pass through native worker routing before leader-session raw absorption, legitimate stop gates, the boundary that milestone reporting must not replace continued execution, and the boundary that shared-board-specific coordination semantics stay outside Main RULES scope |
...
- [ ] Native worker routing remains workload-shaped, uses the smallest effective worker structure for broad/high-context work, and keeps custom-agent selection as candidate selection after routing rather than routing ownership
