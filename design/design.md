...
| 30 | todo-standards.md | todo-standards.design.md v2.11 | Durable TODO governance with startup-establishment bridge, current-phase-first built-in task-list usage for non-trivial live execution tracking, same-objective reuse/append retention, completed-task visibility until closure, task-list-first next-work discovery with bounded fallback to broader execution surfaces, deferral of multi-session shared-board coordination protocol details to the first-class coordination owner, explicit request-style handoff naming that keeps sender phase out of default visible request titles, and clearer visible session identity for session-held board work |
...
| 40 | shared-execution-coordination.md | shared-execution-coordination.design.md v1.2 | First-class owner for multi-session shared execution coordination, including shared-board semantics, visible session-held task identity, session lease/handoff lifecycle, request-layer vs execution-layer separation, receiving-side phase ownership, continuity-first retention, retention matrix semantics, optional memsearch operating guidance, and future-optional peer-messaging boundaries |
