...
### 3.1 Active Runtime Inventory (37 Rules)
...
| 2 | answer-presentation.md | answer-presentation.design.md v1.19 | Principle-first, trigger-driven presentation guidance for readable, orderly, and scannable output, including purpose-first framing near the start of diagnosis/test/recommendation/proposal/update answers, defer of ordinary answer-table semantics to the central `table-format-and-usage` owner, scoped local-fact presentation for machine-specific values in snapshots, compact governing-basis clarification blocks for materially outcome-changing basis ambiguity, compact post-compact re-anchor blocks for compacted-session continuation, compact memory-status blocks that distinguish remembered path-scoped context from freshly checked current-state facts, narrower next-stage blocks that do not interrupt active execution, compact variable-role structures for identifier-heavy explanations, compact near-term gloss support for abstract internal phrasing, and compact proposal layouts that distinguish advisory future-work ideas from queued execution |
...
| 5 | artifact-initiation-control.md | artifact-initiation-control.design.md v1.2 | Startup-governance owner that resolves design/changelog/TODO/phase/patch posture before meaningful governed work drifts, initializes live task tracking early when non-trivial work needs visible execution state, and keeps patch non-default during greenfield startup unless a real before/after review surface exists |
| 6 | authority-and-scope.md | authority-and-scope.design.md v2.1 | User authority, deterministic precedence, fresh-directive override behavior, a rule against unnecessary option branching when one safe continuation path already exists, an explicit boundary that future-work proposals remain advisory until selected, a RULES-first-over-memory boundary when the user explicitly declares the governing fix belongs in RULES, a memory-governance deferral plus current-scope-wins boundary for non-matching path-scoped remembered context, a user-owned governing-basis selection boundary when materially different policies/frames remain unresolved, a post-compact re-anchor boundary that preserves the active objective and active frame after compact, and a team-expansion boundary for overlapping teammate roles |
...
| 22 | phase-implementation.md | phase-implementation.design.md v2.7 | First-class semantic standard for phased execution planning with early phase-establishment bridge and explicit phase-to-patch linkage when patch is in scope |
| 23 | project-documentation-standards.md | project-documentation-standards.design.md v2.16 | Repository-level document-role model plus startup artifact gate, explicit patch-linkage verification for phased work, a clarified live-task-list-vs-durable-TODO tracking split, non-default startup patch posture for greenfield baseline formation, portable public onboarding/install guidance by default, portable-by-default package-local support assets when they are reusable source content, and support-layer modeling for the optional `plugin/**` extension package area |
...
| 30 | todo-standards.md | todo-standards.design.md v2.4 | Durable TODO governance with startup-establishment bridge plus proactive built-in task-list usage for non-trivial live execution tracking |
...
| 36 | table-format-and-usage.md | table-format-and-usage.design.md v1.0 | First-class owner for ordinary answer-table usage, the selected light plain aligned no-frame default style, list-versus-table boundary, bounded markdown-table exceptions, and table anti-pattern semantics |
| 37 | zero-hallucination.md | zero-hallucination.design.md v1.3 | Verified information only with fact/inference/hypothesis separation and scoped non-finding discipline |
...
| Presentation & Readability | answer-presentation, explanation-quality, flow-diagram-no-frame, natural-professional-communication, table-format-and-usage | Readable, orderly, scannable, naturally professional, and table-consistent output presentation |
