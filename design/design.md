...
### 3.1 Active Runtime Inventory (39 Rules)
...
| 30 | todo-standards.md | todo-standards.design.md v2.6 | Durable TODO governance with startup-establishment bridge, current-phase-first built-in task-list usage for non-trivial live execution tracking, and support for continued execution when the active path remains clear |
...
| User Control | authority-and-scope, custom-agent-selection-priority, emergency-protocol, functional-intent-verification, operational-failure-handling, refusal-classification, recovery-contract, runtime-topology-control, execution-continuity-and-mode-selection | Preserve user authority, correct mode selection, continuous execution when the path is clear, and safe operational posture |
