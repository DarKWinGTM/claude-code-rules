...
| 10 | document-changelog-control.md | document-changelog-control.design.md v4.9 | Chain authority, metadata, synchronization contract, README-current-state-versus-changelog-history split, and inactive `changelog/done/` completed-history surface |
...
| 19 | memory-governance-and-session-boundary.md | memory-governance-and-session-boundary.design.md v1.6 | First-class owner for memory role boundaries, root `MEMORY.md` index-only behavior, loader-warning and index-bloat maintenance triggers, compact-index recovery guidance, `global/path/archive` taxonomy, path-primary applicability, session provenance, canonical `SCOPE.md`, archive-inactive lifecycle semantics, and generic optional external recall guidance that stays supplemental and subordinate to stronger execution evidence without implying a Main RULES-managed custom skill path or retired bridge mechanism |
...
| 39 | execution-continuity-and-mode-selection.md | execution-continuity-and-mode-selection.design.md v1.15 | First-class owner for discussion-vs-execution mode selection, startup-gate-first execution boundaries, continuous-execution defaults, active next-work discovery, goal-state continuity during non-trivial work, completion-to-next-goal bridge behavior, verification continuation, phase-lineage continuity, capture-before-continue protection, worker routing before broad continuation, and legitimate stop gates |
