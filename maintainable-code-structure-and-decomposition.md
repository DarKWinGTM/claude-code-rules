...
> **Current Version:** 1.0
> **Design:** [design/maintainable-code-structure-and-decomposition.design.md](design/maintainable-code-structure-and-decomposition.design.md) v1.0
...
---
## Rule Statement
**Core Principle: During coding and refactoring, preserve maintainable code structure by assigning responsibilities clearly, decomposing only when it improves real changeability, and preventing unnecessary God functions, God files, hidden dependencies, and wrong abstractions without enforcing rigid line-count or architecture-template rules.**
This rule owns coding-time responsibility boundaries, smell-triggered decomposition, smallest-useful-structure decisions, anti-overengineering guardrails, and behavior-preserving refactor posture. It does not replace tactical/strategic convergence, project-specific architecture, testing ownership, evidence wording, or user-directed implementation choices.
---
## Scope Boundary
...
พูดง่าย ๆ: AI ควรเขียนโค้ดให้คนแก้ต่อได้ง่าย แต่ไม่ใช่แยกไฟล์/สร้าง layer เพียงเพราะอยากให้ดูเป็น architecture.
---
## Core Contract
...
- do not treat visual neatness, shorter files, or more layers as maintainability proof by themselves
...
Required guidance:
- treat God function, God file, long method, large class, shotgun surgery, divergent change, feature envy, primitive obsession, hidden dependency, and speculative generality as reasons to inspect structure
- do not refactor solely because a file or function is long
- decide from cohesion, coupling, change axes, testability, and implementation risk
...
Required guidance:
- prefer clear local extraction before new layers
...
### 6) Explicit dependency and state-boundary principle
...
### 7) Behavior-preserving refactor principle
...
### 8) Tactical/strategic integration principle
This rule shapes code structure; `tactical-strategic-programming.md` shapes tactical entry and strategic convergence.
Required guidance:
- tactical fixes may stay local, but should not worsen God-file or God-function drift without a reason
- if a tactical slice leaves structure debt, state whether it will be absorbed, promoted, replaced, or retired later
- strategic work should use this rule to define stable responsibility boundaries rather than accumulating tactical fragments
---
## Decomposition Decision Gate
Before expanding or introducing a substantial function, file, module, class, or abstraction, decide the smallest fitting structure:

| Situation | Preferred action |
|---|---|
| cohesive, small, one reason to change | keep local and clear |
| repeated named step inside one flow | extract a helper/function with a clear name |
...
| Long method | sequence is hard to scan or verify | extract named steps when names improve flow understanding |
...
5. avoid speculative abstractions and wrong DRY extractions
6. preserve behavior during refactor unless behavior change is explicit
7. verify with relevant checks when available and report verification limits honestly
---
...
- [project-documentation-standards.md](project-documentation-standards.md) - governs durable docs when structural design changes require documentation
