...
> **Design:** [design/answer-presentation.design.md](design/answer-presentation.design.md) v1.28
> **Session:** d42465eb-30a7-4bc8-b9d6-03e52306e9a5
> **Full history:** [changelog/answer-presentation.changelog.md](changelog/answer-presentation.changelog.md)
---
## Rule Statement
**Core Principle: Present answers so they are orderly, readable, easy to scan, and complete enough without over-expansion, while keeping structure flexible enough to match the user's actual need, expose proportional goal/output/gate framing when useful, and show meaningful next recommendations at true completion boundaries.**
This rule owns layout and scanability, including compact goal-aware presentation shapes. It does not replace reasoning quality, truthfulness, verification, evidence wording, goal-set semantics, or execution-continuity ownership.
---
## Core Principles
...
| Goal-aware working frame | non-trivial work needs orientation, verification focus, or closeout clarity | compact goal, expected output, and completion gate without forcing a template |
| Roadmap-aware completion | active objective truly complete and checked successor work exists | recommended next phase/wave/goal, why next, goal, output, gate |
...
| phase/progress or closeout | plain-language opening plus delivery/feature/impact/verification/next-state grouping, with roadmap-aware next-goal recommendation when meaningful |
| goal-set review or goal-aware work | `Goal` / `Output` / `Gate` only when useful, or `Main goals now` / `Current focus` / `Why rebalance` for broader goal review |
...
| goal-set review | `Main goals now` / `Current focus` / `Why rebalance`; use `Goal` / `Output` / `Gate` only when non-trivial work benefits from it |
...
- **Goal-aware working frame:** use `Goal`, `Output`, and `Gate` when non-trivial work needs orientation, drift control, or verification clarity; keep it internal or omit it when the path is obvious.
- **Roadmap-aware completion:** after true objective completion, use `Recommended next`, `Why this next`, `Goal`, `Output`, and `Gate` when checked roadmap/goal surfaces support successor work and selected safe continuation is not already underway.
...
| completed phase hides meaningful next-phase/goal recommendation | add a roadmap-aware completion block with why/goal/output/gate |
| goal/output/gate block appears in every simple answer | keep goal framing triggered and proportional, not mandatory |
...
Allowed: short answers may stay short; headings, tables, snapshots, scope blocks, goal-aware frames, full-set blocks, optional deep-dive offers, and next-stage blocks are optional unless they improve comprehension; structure may vary as long as readability, scanability, and semantic formatting remain strong.
Not allowed: using flexibility to keep complex answers disorganized; using formatting as decoration; obscuring sequence, comparison, scope, goal/output/gate, or section purpose; using snapshot tables as a substitute for orientation or implication; defaulting to a narrow slice when the full set should be visible; defaulting to deeper same-scope explanation when the answer should move forward; forcing goal blocks into simple answers; pruning a meaningful roadmap-aware next-goal recommendation after true completion.
---
## Quality Metrics
| Metric | Target |
|---|---|
...
| Diagnostic, scope, goal-aware frame, full-set, roadmap-aware completion, optional deep-dive, next-stage, and compact-table usefulness | high when relevant |
...
- [response-closing-and-action-framing.md](response-closing-and-action-framing.md) - closing synthesis, recommendations, alternatives, roadmap-aware next-goal completion, optional deep-dive offers, and advisory proposals
- [goal-set-review-and-priority-balance.md](goal-set-review-and-priority-balance.md) - goal-aware working frame semantics, goal hierarchy, and next-goal recommendation boundaries
- [explanation-quality.md](explanation-quality.md) - explanation flow, easy-first compactness, goal-frame readability, full-set-first logic, and stage progression
